...
<div class="h-screen flex overflow-hidden">
  @include('partials.admin-sidebar', ['activeNav' => 'teachers'])
  ...
  <div class="flex-1 flex flex-col min-w-0 h-full">
    <header class="shrink-0 h-[68px] border-b border-border topbar-blur flex items-center gap-3 px-5 sm:px-7">
      ...
      <h1 class="text-[19px] font-bold leading-tight">Formateurs</h1>
    ...
    <main class="flex-1 overflow-y-auto scroll-area">
      <div class="max-w-[900px] mx-auto px-5 sm:px-7 py-7">
        <section class="card overflow-hidden">
          <div class="flex items-center gap-3 px-5 py-4 border-b border-border">
            <h3 class="text-[15.5px] font-bold">Tous les formateurs</h3>
            <span class="badge">{{ $teachers->count() }}</span>
          </div>
          @if($teachers->isEmpty())
            <div class="px-5 py-12 text-center text-muted text-[14px]">Aucun formateur pour le moment.</div>
          @else
          <div class="divide-y divide-border">
            @foreach($teachers as $teacher)
            @php
              $colors = ['#3f6b8a','#7a5c46','#5a6b4a','#8a4f6b','#6b5a4a','#4a6b5a','#8a6b3a'];
              $tc = $colors[abs(crc32($teacher->name)) % count($colors)];
              $parts = preg_split('/\s+/', trim($teacher->name));
              $ti = mb_strtoupper(mb_substr($parts[0], 0, 1)) . (count($parts) > 1 ? mb_strtoupper(mb_substr(end($parts), 0, 1)) : '');
            @endphp
            <div class="flex items-center gap-4 px-5 py-4 hover:bg-surface-2 transition">
              <span class="avatar" style="background: {{ $tc }}">{{ $ti }}</span>
              <div class="flex-1 min-w-0">
                <div class="font-semibold text-[14.5px]">{{ $teacher->name }}</div>
                <div class="text-[12.5px] text-faint">{{ $teacher->email }}</div>
              </div>
              <div class="flex items-center gap-4 text-[13px] text-muted shrink-0">
                <span class="tnum"><b class="text-ink">{{ $teacher->taught_courses_count }}</b> matière{{ $teacher->taught_courses_count != 1 ? 's' : '' }}</span>
                <span class="tnum"><b class="text-ink">{{ $teacher->classes_as_teacher_count }}</b> classe{{ $teacher->classes_as_teacher_count != 1 ? 's' : '' }}</span>
              </div>
            </div>
            @endforeach
          </div>
